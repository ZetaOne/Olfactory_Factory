TITLE neurorishika_inhibitory_without_adaptation.mod
 ...
 ***************************************************************************
 ...
 changes:
  ...
  SAME FILE AS NR_inhibitory just stored calcium levels have been set to zero (CaCinf = 0.000)
...
DERIVATIVE states {  
        ...
        n' = (ninf-n)/ntau
        m' = (minf -m)/mtau            
        CaC' = 0                                :(CaCinf -CaC)/CaCtau - 0.0002*ica
        mk' = 0                                 :((0.01*CaC/(0.01*CaC + 0.02)) - mk )*1.28*(0.02 +0.01*CaC )
}
